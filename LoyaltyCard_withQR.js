...
<div class="card">
  ...
  tpl_basic_full: function (card) {
    ...
    if (showBarcode) {
      ...
      // QR offline: render inside detail view using UTF8 blocks (no canvas, no network)
      contentBlock = `
        <div class="barcodePanel">
          <div id="qrArea" class="qrArea"></div>
          <div class="codeText" id="codeText" style="word-break:break-all"></div>
        </div>`;
    } else {
...
(function(){
  ...
  if (showBarcode) {
    if(fmt === "ean13"){
      ...
      if(ct) ct.textContent = ean13_spacer(r.ok ? r.digits13 : val);
    } else if(fmt === "code128"){
      renderCode128B("barcodeArea", val, ${BAR_C128_HEIGHT}, ${BAR_C128_MODULE_W});
      var ct2 = document.getElementById("codeText");
      if(ct2) ct2.textContent = val;
    } else {
      var ba = document.getElementById("barcodeArea");
      if(ba) ba.innerHTML = "<div style='font:14px sans-serif;color:#111;'>Unsupported format</div>";
      var ct3 = document.getElementById("codeText");
      if(ct3) ct3.textContent = val;
    }
    return;
  }
